...
def _upsample_like(x, size):                                                # 定义上采样
    return nn.Upsample(size=size, mode='bilinear', align_corners=False)(x)  # 上采样作者采用了双线性插值模式
...
def _size_map(x, height):                                                   
    ...
    for h in range(1, height):
        sizes[h] = size
        size = [math.ceil(w / 2) for w in size]                             # 向上取整
    return sizes


class REBNCONV(nn.Module):                                                  # 同u2net.py的REBNCONV类
...
class RSU(nn.Module):
    ...
    def forward(self, x):                                                   # 前向函数
        sizes = _size_map(x, self.height)                                   # 此处的height为RSU内的小Unet有多少层
        x = self.rebnconvin(x)

        # U-Net like symmetric encoder-decoder structure
        def unet(x, height=1):
            if height < self.height:
                x1 = getattr(self, f'rebnconv{height}')(x)                  # gatatter用于返回对象属性值
